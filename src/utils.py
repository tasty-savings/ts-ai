...
def get_user_info(user_allergy_ingredients, user_dislike_ingredients, user_spicy_level, user_cooking_level, user_owned_ingredients, user_basic_seasoning, must_use_ingredients):
    """
        유저 정보를 받아오는 함수 (이후 body로 받게 됨)
        유저의 알레르기 있는 식재료, 싫어하는 식재료, 매운 맛을 좋아하는 정도, 요리 숙련도, 보유한 재료, 기본 조미료, 반드시 사용해야 하는 식재료를 포함한 유저 정보를 반환합니다.
        return: dict
    """
    user_info = {}
    user_info["user_allergy_ingredients"] = user_allergy_ingredients       # 유저의 알레르기 있는 식재료
    user_info["user_dislike_ingredients"] = user_dislike_ingredients       # 유저가 싫어하는 식재료
    user_info["user_spicy_level"] = user_spicy_level                       # 매운 맛을 좋아하는 정도
    user_info["user_cooking_level"] = user_cooking_level                   # 유저의 요리 숙련도
    user_info["user_owned_ingredients"] = user_owned_ingredients           # 보유한 재료
    user_info["user_basic_seasoning"] = user_basic_seasoning               # 유저가 가진 기본 조미료
    user_info["must_use_ingredients"] = must_use_ingredients               # 반드시 사용해야 하는 식재료
